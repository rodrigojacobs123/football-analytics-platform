...
def load_player_events_season(
    league: str,
    season: str,
    player_id: str,
    type_ids: tuple[int, ...] | None = None,
) -> pd.DataFrame:
    """Scan all partidos files and extract events for a single player.

    Cached per (league, season, player_id, type_ids). First call is slow;
    subsequent calls for the same player return instantly.

    Args:
        league: League folder name.
        season: Season string (e.g. "2025-2026").
        player_id: Opta playerId to filter.
        type_ids: If provided, only return events with these typeIds.
    ...
    for f in sorted(p_dir.glob("*.json")):
        ...
        for e in events:
            ...
            if type_ids and tid not in type_ids:
                continue
            quals = {q["qualifierId"]: q.get("value", "1")
                     for q in e.get("qualifier", [])}
            rows.append({
                "typeId":    tid,
                "x":         float(e.get("x", 0)),
                "y":         float(e.get("y", 0)),
                "end_x":     float(quals.get(140, e.get("x", 0))),
                "end_y":     float(quals.get(141, e.get("y", 0))),
                "outcome":   int(e.get("outcome", 0)),
                "periodId":  int(e.get("periodId", 1)),
                "timeMin":   int(e.get("timeMin", 0)),
                "xg":        float(quals[395]) / 100 if 395 in quals else None,
                "is_penalty": 9 in quals,
                "is_header":  15 in quals,
                "zone":      quals.get(56, ""),
                "pass_len":  float(quals.get(140, 0)) if tid == 1 else None,
                "angle_deg": float(quals.get(213, 0)) if 213 in quals else None,
            })
