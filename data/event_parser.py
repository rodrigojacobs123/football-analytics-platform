...
def extract_formation(events: list[dict], team_id: str) -> dict | None:
    ...
    for e in events:
        ...
        for i, pos in enumerate(formation_vals):
            ...
            # order_in_row: index of this player within their position row.
            # Opta lists players right-to-left within each row (RB→LB, RW→LW),
            # so order_in_row=0 = rightmost player in that row.
            order = row_counters.get(pos, 0)
            row_counters[pos] = order + 1
            entry = {
                "player_id": pid,
                "shirt": shirt,
                "position_row": pos,
