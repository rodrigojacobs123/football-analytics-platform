...
from viz.kpi_cards import section_header, kpi_card, metric_highlight, page_header, mu_section
...
with st.expander("**⚡ Player Intelligence Card** (Opta-style deep profile)", expanded=True):
    ...
    # ── Header ────────────────────────────────────────────────────────────
    from config import MU_GOLD, MU_RED, MU_DARK_BG
    st.markdown(f"""
        ...
        <div style="display:flex;justify-content:space-between;align-items:flex-start;">
            <div>
                ...
                <div style="margin-top:0.6rem;">
                    <span style="color:{MU_RED};font-weight:700;font-size:1rem;">'{archetype_name}'</span>
                    <span style="color:#aaa;font-size:0.85rem;margin-left:0.4rem;">→ {archetype_desc}</span>
                </div>
            </div>
            <div style="text-align:right;">
                <span style="color:{MU_GOLD};font-size:2rem;font-weight:700;">{ovr}</span>
...
def _dim_pill(dim_name: str, pos_key: str) -> str:
    accent = MU_RED if pos_key == "Forward" else (
        "#4A90D9" if pos_key == "Midfielder" else (
        MU_GOLD if pos_key == "Defender" else "#5FBF7A"
    ))
    return (
        ...
        with tab:
            ...
            for i, (rule, (arch_name, arch_desc)) in enumerate(archetypes):
                ...
                if rule:
                    for dim, thresh in rule.items():
                        ...
                        threshold_html += (
                            ...
                            f' <span style="color:{MU_RED};letter-spacing:1px;">'
                            f'{squares}</span></div>'
                        )
                else:
                    threshold_html = (
                ...
                card = (
                    ...
                    f'border-left:3px solid {MU_GOLD};">'
                    f'<span style="color:{MU_GOLD};font-weight:700;font-size:0.95rem;">'
                    f'{arch_name}</span>'
