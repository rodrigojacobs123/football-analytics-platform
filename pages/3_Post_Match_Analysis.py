...
v3_section_header("xG FLOW", "Momentum & Key Events")
...
# ═══════════════════════════════════════════════════════════════════════════
v3_section_header("TACTICAL BREAKDOWN", "Tactical Overview")
...
# ═══════════════════════════════════════════════════════════════════════════
v3_section_header("FORMATIONS", "Shape & Structure")
...
# ═══════════════════════════════════════════════════════════════════════════
v3_section_header("GOAL ANALYSIS", "Build-Up Sequences")
...
# ═══════════════════════════════════════════════════════════════════════════
v3_section_header("TERRITORY", "Territorial Dominance")
...
# § 5  PRESSING ANALYSIS
# ═══════════════════════════════════════════════════════════════════════════
v3_section_header("PRESSING", "Pressing Analysis")
...
with pcol2:
    ...
    <div style="padding:0.8rem;background:#1A1A2E;border-radius:8px;font-size:0.8rem;color:#999;">
        ...
        <p style="margin:0.5rem 0 0;color:#666;font-size:0.7rem;">
            ...
            tackles + interceptions + challenges + fouls in that zone.
        </p>
    </div>
    """, unsafe_allow_html=True)
...
# ── Transition KPI cards ───────────────────────────────────────────────────
hk1, hk2 = st.columns(2)
with hk1:
    st.markdown(f"##### {home_team}")
    st.markdown(transition_kpi_html(home_rep["transitions"], ppda=ppda_home),
                unsafe_allow_html=True)
with hk2:
    st.markdown(f"##### {away_team}")
...
with fc1:
    st.plotly_chart(
        transition_matrix(home_rep["chain"], home_id,
...
xc2.metric("Differential",
           f"{home_xt['total_xt'] - away_xt['total_xt']:+.2f}")
xc3.metric(f"{away_team} xT", f"{away_xt['total_xt']:.2f}")

xt1, xt2 = st.columns(2)
with xt1:
    st.plotly_chart(
        xt_pitch_heatmap(home_xt["passes_df"],
                         title=f"{home_team} · xT-creating zones"),
        use_container_width=True)
    st.plotly_chart(
...
with xt2:
    st.plotly_chart(
        xt_pitch_heatmap(away_xt["passes_df"],
                         title=f"{away_team} · xT-creating zones"),
        use_container_width=True)
    st.plotly_chart(
        xt_top_contributors_bar(
            away_xt["top_passers"],
            title=f"{away_team} · Top xT contributors",
            color="#42A5F5"),
        use_container_width=True)


# ═══════════════════════════════════════════════════════════════════════════
# § 5d  PRESSURE METRICS  — StatsBomb / The Analyst conventions
# ═══════════════════════════════════════════════════════════════════════════
v3_section_header("PRESSURE", "Defensive Shape")
st.caption(
    "**Defensive line height** = mean x of the team's tackles + interceptions + "
    "clearances (proxy for back-line position). **Ball recovery height** = mean x "
    "of recoveries. **High turnovers** = recoveries in the opponent's defensive "
    "third. **Shot-ending HTOs** = high turnovers leading to a shot within 10 s."
)

home_pr = compute_pressure_metrics(events, home_id, away_id)
away_pr = compute_pressure_metrics(events, away_id, home_id)


def _pressure_card_html(name: str, m: dict, accent: str) -> str:
    return f"""
    <div style="background:#1A1A2E;border-left:3px solid {accent};
                padding:14px;border-radius:6px;">
      <div style="font-weight:700;color:#FFF;font-size:14px;margin-bottom:8px;">{name}</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:11px;">
        <div><span style="color:#888;">Def. line height</span><br><b style="color:#FFF;font-size:18px;">{m['def_line_height']}</b></div>
        <div><span style="color:#888;">Ball recovery height</span><br><b style="color:#FFF;font-size:18px;">{m['ball_recovery_height']}</b></div>
        <div><span style="color:#888;">High turnovers</span><br><b style="color:#FFF;font-size:18px;">{m['high_turnovers']}</b></div>
        <div><span style="color:#888;">Shot-ending HTOs</span><br><b style="color:#FFF;font-size:18px;">{m['shot_ending_high_turnovers']}</b></div>
        <div><span style="color:#888;">Pressure regains (5s)</span><br><b style="color:#FFF;font-size:18px;">{m['pressure_regains_5s']}</b></div>
        <div><span style="color:#888;">Total recoveries</span><br><b style="color:#FFF;font-size:18px;">{m['total_recoveries']}</b></div>
      </div>
    </div>
    """
...
# ═══════════════════════════════════════════════════════════════════════════
v3_section_header("DEFENSIVE ACTIONS", "Tackles, Interceptions & Duels")
...
# § 6.5  BALL WIN HEIGHT
# ═══════════════════════════════════════════════════════════════════════════
v3_section_header("BALL RECOVERY", "Ball Win Height")
...
# ═══════════════════════════════════════════════════════════════════════════
v3_section_header("ZONE CONTROL", "Dominant Actions by Zone")
...
# ═══════════════════════════════════════════════════════════════════════════
v3_section_header("PROGRESSION", "Progressive Passes")
...
# ═══════════════════════════════════════════════════════════════════════════
v3_section_header("HEATMAPS", "Touch Zones")
...
# ═══════════════════════════════════════════════════════════════════════════
v3_section_header("SET PIECES", "Dead Ball Analysis")
...
# ═══════════════════════════════════════════════════════════════════════════
st.markdown("---")
v3_section_header("PLAYER RATINGS", "Performance Scores")
...
_r_home = compute_match_ratings(events, home_id)
_r_away = compute_match_ratings(events, away_id)
_motm_parts = [df for df in [_r_home, _r_away] if not df.empty]
if _motm_parts:
    _all_merged = pd.concat(_motm_parts, ignore_index=True)
    _motm = _all_merged.loc[_all_merged["rating"].idxmax()]
    _motm_team = home_team if _motm.get("team_id", "") == home_id else away_team
    _motm_color = "#DA291C" if _motm_team == home_team else "#42A5F5"
    _motm_score = float(_motm["rating"])
    _motm_score_color = "#3DD68C" if _motm_score >= 8.0 else "#FBE122"
    st.markdown(
        f'<div style="background:#0B0B11;border:1px solid {_motm_color}30;'
        f'border-left:4px solid {_motm_color};border-radius:12px;'
        f'padding:1rem 1.5rem;margin-bottom:1.2rem;display:flex;align-items:center;gap:1.5rem;">'
        f'<div style="min-width:60px;text-align:center;">'
        f'<div style="font-family:JetBrains Mono,monospace;font-size:0.58rem;color:#555;'
        f'letter-spacing:0.2em;text-transform:uppercase;margin-bottom:0.2rem;">MOTM</div>'
        f'<div style="font-family:Anton,sans-serif;font-size:2.8rem;line-height:1;'
        f'color:{_motm_score_color};">{_motm_score:.1f}</div>'
        f'</div>'
        f'<div style="flex:1;">'
        f'<div style="font-family:Anton,sans-serif;font-size:1.5rem;letter-spacing:0.05em;'
        f'color:#F0F0F0;text-transform:uppercase;">{_motm["player_name"]}</div>'
        f'<div style="font-family:JetBrains Mono,monospace;font-size:0.65rem;color:#555;'
        f'letter-spacing:0.15em;text-transform:uppercase;">'
        f'{_motm.get("position", "")} &nbsp;·&nbsp; {_motm_team}</div>'
        f'</div>'
        f'</div>',
        unsafe_allow_html=True,
    )
...
# ═══════════════════════════════════════════════════════════════════════════
st.markdown("---")
v3_section_header("PLAYER DEEP DIVE", "Individual Analysis")
